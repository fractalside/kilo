...
$k.initFieldNotes = function() {
	...
	$(bc.boxHandle).each(function(index2) {$(this).find(bc.itemHandle).each(function(index) {
		...
		if (fnc.ink[ink] != null) { //4 bootstrap
		...
		try {
			...
			for (var i = 0; i < bc.focusable.length; i++) {
				...
				if ($(this).hasClass(item)) { //The li has the class
					$k.log("fieldNotes: focus class " + item + " name(input) " + name + (($(fieldSelector.length)) ? " found" : " not-found"));
					$(fieldSelector).focus();
					$k.v.firstFocus = true;
				} 
			}
		} catch(e) {
			console.log("FOCUSABLE" + e.message + e.stack);
		}
...
$k.mainForm = function() {
	...
	return ($(idForm).length == 0) ? null : handle;
}
...
$k.init = function(config) {
	...
	$k.initMod("fieldNotes", 		$k.initFieldNotes, true);	//After focusFirst
