...
$k.cfg = function(cfg) {return $k.nvl("",$k.c,cfg);}
...
/** **/
$k.allNoneBtn = function(rootName, btnClassName, allSelected, msgAll, msgNone) {
	return {
		className: btnClassName,
		text: 
			'<span id="' + rootName + '_all" 	class="glyphicon glyphicon-check" 	title="' + 
			$k.pm.redact(msgAll) + '" ' + ((allSelected) ? 'style="display:none;"' : '') + '/>'+
			'<span id="' + rootName + '_none" 	class="glyphicon glyphicon-unchecked" title="' + 
			$k.pm.redact(msgNone) + '" ' + ((allSelected) ? '' : 'style="display:none;"') + '/>',
		action: function ( e, dt, node, config ) {
			var checkAll = !$k.allChecked(rootName, dt);
			var name, value, checkItem;
			dt.column(0).nodes().to$().each(function(index) {    
				checkItem = $(this).find('[type=checkbox]');
				name = checkItem.attr("name").slice(0, -2);
				value = checkItem.attr("value");
				$k.v.selects[name][value] = checkAll;
				$(this).closest("tr").toggleClass("selected", checkAll);
				checkItem.prop("checked", checkAll);
			});
			$k.updateInputCountAfterRow(name);
			$k.updateAllNoneBtn(rootName, dt);
			$k.updatePageAfterRow(dt);
		}		
	}
};
...
/** **/
$k.joinTrue = function(sep, array) {
	var list = "";
	$k.v.joinTrueCounter = 0;
	for (var j in array) {
		if (array[j]) {
			list = list  + sep + j;
			$k.v.joinTrueCounter++;
...
/** **/
$k.updateInputCountAfterRow = function(name) {
	var form = $($k.mainForm());
	var hf = form.find("input:" + $k.c.activeRowInputType + "[name=" + name + "]");
	hf.attr("value", $k.joinTrue("#", $k.v.selects[name]));
	$k.v.selectCounter[name] = $k.v.joinTrueCounter;
	$k.pm.log("update counter " + name + "  = " + $k.v.selectCounter[name]);
}

/** **/
$k.allChecked = function(name, dt) {
	if ($k.v.selectCounter[name] == null) {
		$k.pm.log("check counter nulo");
		return false;
	} else {
		var total = dt.rows().count();
		$k.pm.log("check counter " + name + "  = " + $k.v.selectCounter[name] + " / " + total);
		return (total == $k.v.selectCounter[name]);
	}
}

/** **/
$k.updateAllNoneBtn = function(name, dt){
	var allChecked = $k.allChecked(name,dt);
	$("#" + name + "_all").toggle(!allChecked);
	$("#" + name + "_none").toggle(allChecked);
}

/** **/
$k.updatePageAfterRow = function(dt) {
	if ((dt != null) && ($k.c.table.rowCallBack != null)) {
		$k.c.table.rowCallBack(dt);
	}
}
...
$k.initTableActiveRow = function() {
	$("table").each(function() {
		...
		if (checks.length > 0) {
			var name = checks.first().attr("name");
			
			$(checks).each(function(){
				$(this).attr("name", name + "_k");
			...
			var preSelData = $k.cfg("table.preSelData");
			if (preSelData != "") {
				preSelData = $(this).attr("data-" + preSelData);
			}
			if (preSelData != "") {
				preSelData = preSelData.split("#");
				for (var i=0;i<preSelData.length;i++) {
					if (preSelData[i].indexOf("+") == 0) {
						$k.v.selects[name][preSelData[i].slice(1)] = true;
					} else {
						$k.v.selects[name][preSelData[i]] = false;
					}
				}
			}
			//$k.pm.log($k.jsons($k.v.selects));
			var form = $($k.mainForm());
			form.append("<input type='" + $k.c.activeRowInputType + "' name='" + name + "' value='' />");
			$k.updateInputCountAfterRow(name);
		}
	});
	...
	$('table tr').click(function() {
		...
		if ($k.c.bootstrap) {
			...
			$k.updateInputCountAfterRow(name);
			$k.updateAllNoneBtn(name, dt); //after, updatePage modify selectCounter
			$k.updatePageAfterRow(dt);
		} else {
...
$k.init = function(config) {
	...
	$k.initMod("tableActiveRow", $k.initTableActiveRow, 	true);
	...
	$k.initMod("fieldNotes", 		$k.initFieldNotes, ($k.cfg("bulletinList.boxHandle") != "")); //before focustFirst (default option)
	$k.initMod("focusFirstField", 	$k.initFocusFirstField, ($k.c.field.firstJqh != null));
	
	$k.initMod("defaultSubmit", 	$k.initDefaultSubmit, 	($k.c.defaultSubmitHandle != null));
